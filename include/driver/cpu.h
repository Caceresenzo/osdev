...
/* Vendor-strings from big compagnie. */
...
# define CPUID_REQUEST_GETVENDORSTRING	0
# define CPUID_REQUEST_GETFEATURES		1
# define CPUID_REQUEST_GETTLB			2
# define CPUID_REQUEST_GETSERIAL		3

# define CPUID_REQUEST_INTELEXTENDED		0x80000000
# define CPUID_REQUEST_INTELFEATURES		0x80000001
# define CPUID_REQUEST_INTELBRANDSTRING		0x80000002
# define CPUID_REQUEST_INTELBRANDSTRINGMORE	0x80000003
# define CPUID_REQUEST_INTELBRANDSTRINGEND	0x80000004

static inline void	cpuid(int code, uint32 *a, uint32 *d) {
	asm volatile("cpuid":"=a"(*a),"=d"(*d):"a"(code):"ecx","ebx");
}

static inline void	cpuid_string(int code, uint32 where[4]) {
	asm volatile("cpuid":"=a"(*where),"=b"(*(where+1)), "=c"(*(where+2)),"=d"(*(where+3)):"a"(code));
}

char				*cpuid_vendor_get(char *vendor);
